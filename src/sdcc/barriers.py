...
def plot_contour_sweep(
    ...
    plt.plot(
        np.degrees(theta_coords),
        np.degrees(phi_coords),
        "r*",
        markersize=10,
        markeredgecolor="k",
    )
    ...
    plt.annotate(
        "Energy Barrier: " + str(int(threshold - start_energy)) + " Jm$^{-3}$",
        (0.5, 0.9),
...
def contour_sweep(
    thetas, phis, energies, start_energy, incr, plot=False, plot_dir=None
):
    """
...
def check_min_dist(thetas, phis, energies, threshold, min_angle):
    ...
    for i, j in combinations(np.arange(len(theta_coords)), 2):
        angle = great_circle_dist(
            theta_coords[i], theta_coords[j], phi_coords[i], phi_coords[j]
        )
        if angle < min_angle:
            return True
        else:
...
def find_energy_barrier(
    ...
    best_coords, best_theta, best_phi, best_energy = find_global_minimum(
        thetas, phis, energies
    )
    ...
    saddle = contour_sweep(
        thetas, phis, energies, start_energy, incr, plot=plot, plot_dir=plot_dir
    )
    ...
    while min_check:
        saddle = contour_sweep(
            thetas, phis, energies, saddle, incr, plot=plot, plot_dir=plot_dir
        )
