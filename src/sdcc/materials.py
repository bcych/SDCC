    ...
    def make_functions(self,param):
        if self.solidSolution:
            ...
            for k in self.anisotropyDict.keys():
                #For some reason this is the function for k2 twice?
                #why would that happen
                func_dict[k] = lambda T, Tc, const=k: self.anisotropyDict[const](comp, T, Tc)
            anis_type = type(self)
...
class CubicAnisotropy(Anisotropy):
    def __init__(self,k1,k2):
        anisotropyDict={'k1':k1, 'k2':k2}
        super().__init__(anisotropyDict)
