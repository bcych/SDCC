...
def plot_net(ax=None):
    ...
    ax.axis("off")
    Dcirc = np.radians(np.arange(0, 361.))
...
levels=10,n_points=100,projection='equirectangular'):
    ...
    thetas,phis,energies=energy_surface(k1,k2,rot_mat,Ms,LMN,
    ext_field=ext_field,n_points=n_points)
    ...
    #If stereonet, plot on two stereonets (upper, lower)
        ...
        xs,ys=dimap(thetas[phis>=0].flatten(),
        phis[phis>=0].flatten()).T
...
**kwargs):
    ...
    if 'equi' in projection.lower():
        ...
        try:
            saddle_x,saddle_y=dimap(saddle_thetas[cs],
            saddle_phis[cs]).T
        except:
            saddle_x,saddle_y=dimap(saddle_thetas[cs],
            saddle_phis[cs])
